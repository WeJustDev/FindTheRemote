...
Shader "Water/Ocean_06" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _ShineTexture; uniform float4 _ShineTexture_ST;
            struct VertexInput {
...
////// Emissive:
                ...
                float2 node_7411 = (node_9870+node_4825.g*float2(0,0.05));
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(node_7411, _MainTexture));
                float2 node_5898 = (node_9870+node_4825.g*float2(0,0.01));
                float4 _Addittionalfoam_var = tex2D(_Addittionalfoam,TRANSFORM_TEX(node_5898, _Addittionalfoam));
                float2 node_2331 = (i.uv0+node_4825.g*float2(-0.05,-0.05));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_2331, _Noise));
                float node_4471 = _Noise_var.r;
                float2 node_2716 = (lerp(node_1855,float2(node_4471,node_4471),_Noiselevel)+node_4825.g*float2(0,0.02));
                float4 _FoamTexture_var = tex2D(_FoamTexture,TRANSFORM_TEX(node_2716, _FoamTexture));
                float node_1598 = 1.0;
                float node_4652 = 1.0;
                float2 node_1054_tc_rcp = float2(1.0,1.0)/float2( node_1598, 2.0 );
                float node_1054_ty = floor(node_4652 * node_1054_tc_rcp.x);
                float node_1054_tx = node_4652 - node_1598 * node_1054_ty;
                float2 node_1054 = (float2((sceneUVs.x * 2 - 1)*(_ScreenParams.r/_ScreenParams.g), sceneUVs.y * 2 - 1).rg + float2(node_1054_tx, node_1054_ty)) * node_1054_tc_rcp;
                float2 node_3076 = lerp(node_1054,float2(_Noise_var.r,_Noise_var.r),_Noiselevel);
                float4 _ShineTexture_var = tex2D(_ShineTexture,TRANSFORM_TEX(node_3076, _ShineTexture));
                float node_4196 = saturate((saturate(max(saturate((1.0-(1.0-_FoamTexture_var.r)*(1.0-_WaterOpacity))),_ShineTexture_var.r))*_Addittionalfoam_var.r));
                ...
                float3 emissive = saturate((1.0-(1.0-saturate(max(saturate(( _WaterColor.rgb > 0.5 ? (1.0-(1.0-2.0*(_WaterColor.rgb-0.5))*(1.0-saturate((_MainTexture_var.rgb*_Addittionalfoam_var.rgb)))) : (2.0*_WaterColor.rgb*saturate((_MainTexture_var.rgb*_Addittionalfoam_var.rgb))) )),saturate((saturate(( node_4196 > 0.5 ? (1.0-(1.0-2.0*(node_4196-0.5))*(1.0-_FoamTexture_var.rgb)) : (2.0*node_4196*_FoamTexture_var.rgb) ))*_FoamOpacity)))))*(1.0-(float3(1,1,1)*node_7739))));
                float3 finalColor = emissive;
                return fixed4(finalColor,saturate((1.0-(1.0-saturate((1.0-(1.0-node_4196)*(1.0-saturate((sceneZ-partZ)/_WaterDepth)))))*(1.0-node_7739))));
            }
            ENDCG
        }
    }
